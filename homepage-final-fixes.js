(() => {
  ...
  const spotifyEmbed = 'https://open.spotify.com/embed/artist/6ozcOAnRAUPn3z5c0GR5kU?theme=0&utm_source=generator';
  ...
  const resumeBackgroundAfterExternalMedia=()=>{if(!externalMediaPlaying)return;externalMediaPlaying=false;if(!bgWasPlaying)return;bgWasPlaying=false;startBackgroundMusic();};

  const startBackgroundMusic=()=>{
    const a=getBgAudio(); if(!a||externalMediaPlaying)return;
    a.src='/assets/other/sound/Background.mp3';
    a.loop=true;
    a.autoplay=true;
    a.preload='auto';
    a.volume=0.65;
  ...
  const ensureCalmPlayer=()=>{
    ...
    if(!player){
      ...
      player.style.cssText='position:relative;z-index:20;width:min(92vw,900px);margin:2rem auto 1.5rem;padding:.65rem .8rem;display:flex;align-items:center;gap:.7rem;background:rgba(0,0,0,.72);border:1px solid rgba(255,255,255,.2);border-radius:12px;box-sizing:border-box;';
      ...
      if(footer) footer.insertBefore(player,footer.firstChild);
      else document.body.appendChild(player);
      player.querySelector('[data-calm-toggle]').onclick=()=>{if(audio.paused)startBackgroundMusic();else audio.pause();};
      ...
      audio.addEventListener('play',()=>player.querySelector('[data-calm-toggle]').textContent='❚❚');
      audio.addEventListener('pause',()=>player.querySelector('[data-calm-toggle]').textContent='▶');
    }
  };

  const ensureSpotifyPlayer=()=>{
    const section=Array.from(document.querySelectorAll('section,div')).find(el=>/SPOTIFY WEB PLAYER/i.test(el.textContent||'') && /LISTEN NOW/i.test(el.textContent||''));
    if(!section)return;
    let frame=section.querySelector('iframe[data-lil-synn-spotify]');
    if(!frame){
      const oldLink=section.querySelector('a[href*="open.spotify.com"]');
      frame=document.createElement('iframe');
      frame.dataset.lilSynnSpotify='true';
      frame.src=spotifyEmbed;
      frame.title='LIL SYNN Spotify player';
      frame.setAttribute('allow','autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture');
      frame.setAttribute('allowfullscreen','');
      frame.loading='lazy';
      frame.style.cssText='display:block;width:100%;height:352px;border:0;border-radius:12px;margin:1rem 0;';
      if(oldLink)oldLink.replaceWith(frame);else section.appendChild(frame);
    }
  };

  const bindSpotifyPlayers=()=>{
    document.querySelectorAll('iframe[src*="open.spotify.com"]').forEach(frame=>{
      frame.setAttribute('allow','autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture');
      frame.setAttribute('allowfullscreen','');
    });
  };
  ...
  const init=()=>{
    fitBackgroundVideo();cleanArt();ensureSingleLS();ensureMerchButton();bindRefresh();ensureCalmPlayer();ensureSpotifyPlayer();bindSpotifyPlayers();bindYouTubePlayers();
    startBackgroundMusic();
    window.addEventListener('load',startBackgroundMusic,{once:true});
    window.addEventListener('resize',fitBackgroundVideo,{passive:true});window.addEventListener('orientationchange',fitBackgroundVideo,{passive:true});
    new MutationObserver(()=>{fitBackgroundVideo();cleanArt();ensureSingleLS();ensureMerchButton();bindRefresh();ensureCalmPlayer();ensureSpotifyPlayer();bindSpotifyPlayers();bindYouTubePlayers();}).observe(document.body,{childList:true,subtree:true});
  };
