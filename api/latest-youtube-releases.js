export default async function handler(req,res){
  ...
  try{
    const response=await fetch(page,{headers:{'user-agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/131 Safari/537.36','accept-language':'en-US,en;q=0.9'}});
    if(!response.ok)throw new Error(`YouTube releases page returned ${response.status}`);
    const html=await response.text();
    const ids=[...new Set([
      ...html.matchAll(/(?:"videoId"\s*:\s*"|videoId\s*"\s*:\s*"|\/watch\?v=)([A-Za-z0-9_-]{11})/g)
    ].map(m=>m[1]))];
    if(!ids.length)throw new Error('YouTube releases page did not expose video IDs to the server');
    const candidates=ids.slice(0,60),items=[];
    for(let i=0;i<candidates.length;i+=6){
      const batch=candidates.slice(i,i+6);
      const results=await Promise.all(batch.map(async id=>{
        try{
          const r=await fetch(`https://www.youtube.com/watch?v=${id}`,{headers:{'user-agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/131 Safari/537.36','accept-language':'en-US,en;q=0.9'}});
          if(!r.ok)return null;
