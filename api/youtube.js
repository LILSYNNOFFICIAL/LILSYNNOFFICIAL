export default async function handler(req,res){
 ...
 const orderItems=items=>[...(items||[])].sort((a,b)=>{const ra=rankTitle(a?.snippet?.title),rb=rankTitle(b?.snippet?.title);if(ra!==rb)return ra-rb;return String(b?.snippet?.publishedAt||'').localeCompare(String(a?.snippet?.publishedAt||''));}).slice(0,11);
