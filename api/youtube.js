export default async function handler(req,res){
 ...
 const fetchRss=async()=>{try{const response=await fetch(`https://www.youtube.com/feeds/videos.xml?channel_id=${CHANNEL_ID}`,{cache:'no-store'});if(!response.ok)return null;const xml=await response.text();const entries=[];for(const m of xml.matchAll(/<entry>([\\s\\S]*?)<\\/entry>/g)){const block=m[1];const videoId=(block.match(/<yt:videoId>([^<]+)<\\/yt:videoId>/)||[])[1]||'';const title=(block.match(/<title>([\\s\\S]*?)<\\/title>/)||[])[1]||'';const published=(block.match(/<published>([^<]+)<\\/published>/)||[])[1]||'';if(videoId)entries.push({kind:'youtube#searchResult',videoId,snippet:{title:title.replace(/&amp;/g,'&').replace(/&quot;/g,'\"').replace(/&#39;/g,"'"),publishedAt:published,description:''}})}return entries.length?entries:null}catch{return null}};
 if(API_KEY){
  try{
