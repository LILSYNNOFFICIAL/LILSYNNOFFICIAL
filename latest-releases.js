...
fetch('/release-catalog.json',{cache:'default'}).then(r=>{if(!r.ok)throw Error('catalog');return r.json()}).then(c=>{
 ...
 root.innerHTML=`<div class="latest-release-head"><p class="section-kicker">RECENT RELEASES</p><h2>THE LATEST SIGNAL</h2><p class="latest-release-sub">The three most recent releases in the canonical LIL SYNN catalog.</p></div><div class="latest-release-grid">${recent.map((title,i)=>{const g=c.groups?.[title],type=(g?.type||'SINGLE').toUpperCase(),art=artFor(title),sp=g?.spotify||c.spotify?.[title]||c.trackSpotify?.[title]||'',ap=g?.apple||c.apple?.[title]||'',detail='/release.html?id='+encodeURIComponent(title);return `<article class="latest-release-card"><a class="latest-release-art-link" href="${detail}" aria-label="Open ${esc(title)} release experience"><img class="latest-release-art" src="${art}" alt="${esc(title)} artwork" loading="eager" decoding="async"></a><div class="latest-release-copy"><span class="latest-release-number">${String(i+1).padStart(2,'0')}</span><span class="latest-release-type">${esc(type)}</span><h3>${esc(title)}</h3><div class="latest-release-actions">${stream('OPEN RELEASE',detail,'latest-release-link-open')}${stream('SPOTIFY',sp,'sp')}${stream('APPLE MUSIC',ap,'am')}${stream('YOUTUBE',youtube,'yt')}</div></div></article>`}).join('')}</div>`;
