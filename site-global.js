(() => {
  ...
  const ensureCalm=()=>{
    ...
    if(!p){
      ...
      audio.addEventListener('timeupdate',()=>{const s=p.querySelector('[data-calm-seek]');if(s&&audio.duration)s.value=audio.currentTime/audio.duration*100});audio.addEventListener('play',()=>{const x=p.querySelector('[data-calm-toggle]');if(x)x.textContent='❚❚'});audio.addEventListener('pause',()=>{const x=p.querySelector('[data-calm-toggle]');if(x)x.textContent='▶'});
    }
    return audio;
  };
