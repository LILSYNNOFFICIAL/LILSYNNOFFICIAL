...
SUNO = ROOT / 'Suno'
CONTENT = SUNO / 'content'

if not SOURCE.exists():
    raise SystemExit(f'Missing source guide: {SOURCE}')

# Rebuild only the generated content layer. Never delete the live /Suno application,
# because that would destroy custom pages, assets, navigation, and the locked Audio Fix V6 module.
CONTENT.mkdir(parents=True, exist_ok=True)
for old in CONTENT.iterdir():
    if old.is_dir():
        shutil.rmtree(old)
    else:
        old.unlink()
(SUNO / 'audio_fix_v6').mkdir(parents=True, exist_ok=True)
(SUNO / 'assets').mkdir(parents=True, exist_ok=True)
...
for src in source_files:
    ...
    dst_md = CONTENT / f'{base}.md'
    dst_html = CONTENT / f'{base}.html'
...
# Legacy fallback files are created only when missing. The live redesigned /Suno pages
# and the locked Audio Fix V6 page must never be overwritten by a central-guide sync.
if not (SUNO / 'index.html').exists():
...
if not (SUNO / 'audio_fix_v6' / 'index.html').exists():
...
if not (SUNO / 'assets' / 'guide.css').exists():
...
if not (SUNO / 'assets' / 'guide.js').exists():
...
print(f'REBUILT {len(manifest)} source documents; preserved live /Suno application files')
