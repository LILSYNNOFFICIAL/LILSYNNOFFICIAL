...
function makeToc(body) {
  const headings = [];
  const withIds = body.replace(/<h([2-3])>([\s\S]*?)<\/h\1>/gi, (full, level, inner) => {
    const text = inner.replace(/<[^>]+>/g,'').replace(/&amp;/g,'&').replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'').toLowerCase();
    if (!text) return full;
    headings.push({ level:Number(level), text:inner, id:text });
    return `<h${level} id="${text}">${inner}</h${level}>`;
  });
  if (!headings.length) return { toc:'', body:withIds };
  const toc = `<aside class="reader-toc"><div class="reader-toc-title">ON THIS PAGE</div><div class="reader-toc-links">${headings.map(h=>`<a class="toc-h${h.level}" href="#${h.id}">${h.text}</a>`).join('')}</div></aside>`;
  return { toc, body:withIds };
}
const searchEntries = [];
...
if (!css.includes(marker)) {
  css += `\n${marker}\n.reader-tools{margin:0 0 36px;border:1px solid var(--line);background:#0b0a08}.reader-jump{padding:16px;border-bottom:1px solid rgba(201,154,62,.1)}.reader-jump>span{display:block;color:var(--gold);font-size:7px;font-weight:900;letter-spacing:.16em;margin-bottom:10px}.reader-jump>div{display:flex;gap:6px;flex-wrap:wrap}.reader-jump a{border:1px solid rgba(201,154,62,.13);padding:7px 9px;color:#827a6f;font-size:7px;letter-spacing:.1em}.reader-jump a:hover{color:var(--gold2);border-color:var(--gold)}.reader-sequence{display:flex;justify-content:space-between;gap:10px;padding:10px 16px}.reader-sequence a{color:#a59d90;font-size:8px;font-weight:800;letter-spacing:.12em}.reader-sequence a:hover{color:var(--gold2)}.reader-sequence a[aria-disabled=true]{opacity:.35;pointer-events:none}.reader-toc{margin:0 0 34px;padding:18px;border:1px solid var(--line);background:rgba(201,154,62,.025)}.reader-toc-title{color:var(--gold);font-size:8px;font-weight:900;letter-spacing:.16em;margin-bottom:12px}.reader-toc-links{display:grid;gap:6px}.reader-toc a{color:#9d9589;font-size:12px}.reader-toc a:hover{color:var(--gold2)}.reader-toc .toc-h3{padding-left:18px;font-size:11px;color:#726b61}.source-status{display:grid;grid-template-columns:1fr 360px;gap:60px;border-top:1px solid var(--line);border-bottom:1px solid var(--line)}.source-status h2{margin:0;font:400 clamp(42px,5vw,68px)/.95 Georgia,serif}.source-status p:not(.eyebrow){color:#8f877b;max-width:720px}.source-status-grid{display:grid;grid-template-columns:repeat(3,1fr);align-content:center;border-left:1px solid var(--line);padding-left:30px}.source-status-grid div{padding:14px 12px;border-bottom:1px solid rgba(201,154,62,.1)}.source-status-grid div:last-child{border-bottom:0}.source-status-grid b{display:block;color:var(--gold2);font:400 30px Georgia,serif}.source-status-grid span{color:#686157;font-size:7px;letter-spacing:.15em}.nav-group.active>.nav-trigger{color:var(--gold2)}.nav-menu a[aria-current=page]{color:var(--gold2);background:rgba(201,154,62,.08)}@media(max-width:760px){.reader-tools{margin-bottom:28px}.reader-jump>div{max-height:180px;overflow:auto}.reader-sequence{flex-direction:column}.source-status{grid-template-columns:1fr;gap:24px}.source-status-grid{border-left:0;border-top:1px solid var(--line);padding:14px 0 0}.source-status-grid div{border-right:1px solid rgba(201,154,62,.1);border-bottom:0}.source-status-grid div:last-child{border-right:0}.reader-toc{padding:14px}}\n`;
  await fs.writeFile(cssPath, css, 'utf8');
}

await fs.writeFile(path.join(root, 'Suno', 'search-index.json'), JSON.stringify({generatedAt:isoDate, syncDate:buildDate, entries:searchEntries}, null, 2), 'utf8');
await fs.writeFile(path.join(root, 'Suno', 'manifest.json'), JSON.stringify({generatedAt:isoDate, syncDate:buildDate, sourceRepository:'canonical Suno V6 source set', sourceDocuments:docs.map(([source])=>source), coreTopics:topics.length, deepDives:deep.length, completeDocuments:docs.length}, null, 2), 'utf8');
console.log(`Generated ${docs.length} complete Suno knowledge-base pages, ${searchEntries.length} searchable source entries and sanitized ${coreFiles.length} core topic pages. Sync date: ${buildDate}.`);
