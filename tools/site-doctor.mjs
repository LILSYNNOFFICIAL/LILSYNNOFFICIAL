...
for(const file of htmlFiles){
  ...
  for(const m of globalCss) if(!/\bid=["']site-global-css["']/i.test(m[0])) fail(`${file}: explicit site-global.css link must use id="site-global-css" to prevent duplicate loading`);
  for(const m of html.matchAll(/(?:src|href)=["']([^"'#?]+)(?:\?[^"']*)?["']/gi)){
    const ref=m[1]; if(!ref.startsWith('/')||ref.startsWith('//')||/^(https?:|mailto:|tel:|data:|javascript:)/i.test(ref))continue;
    const local=ref.slice(1); if(local.includes('['))continue;
    if(!exists(local)) fail(`${file}: missing local asset/reference ${ref}`);
  }
...
if(exists('release-catalog.json')){
  try{const c=JSON.parse(fs.readFileSync('release-catalog.json','utf8'));if(!Array.isArray(c.order)||!c.order.length)fail('release-catalog.json: order is empty');const dup=c.order.filter((x,i,a)=>a.indexOf(x)!==i);if(dup.length)fail(`release-catalog.json: duplicate release order entries: ${[...new Set(dup)].join(', ')}`);if(!c.groups||typeof c.groups!=='object')warn('release-catalog.json: groups object missing');}catch(e){fail(`release-catalog.json: invalid JSON (${e.message})`)}
}
for(const file of jsFiles){try{execFileSync(process.execPath,['--check',file],{stdio:'pipe'})}catch(e){fail(`${path.relative(root,file)}: JavaScript syntax check failed`)}}
...
if(exists('assets/mov/LS_BG_STARS.webm')===false)fail('assets/mov/LS_BG_STARS.webm missing');
if(exists('assets/images/icons/LS_LOGO.png')===false)fail('LS_LOGO.png missing');
if(exists('assets/images/icons/LS_HEADPHONES.png')===false)fail('LS_HEADPHONES.png missing');
