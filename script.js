...
(() => {
  ...
  const localCatalogArt = [{ name: 'Never Known_album_cover.jpg', type: 'file' }, { name: 'home_acoustic_version.png', type: 'file' }, { name: 'I DID IT AGAIN.jpg', type: 'file' }];
  ...
  window.fetch = async (input, init) => {
    ...
    if (/\/api\/latest-youtube-releases(?:\?|$)/i.test(url)) { const response = await nativeFetch('/latest-videos.json', { cache: 'force-cache' }); if (response.ok) { const data = await response.json(); return jsonResponse({ videos: Array.isArray(data.videos) ? data.videos : [] }); } }
...
document.addEventListener("DOMContentLoaded", () => {
  const ham = document.getElementById("hamburger"), menu = document.getElementById("sideMenu"), close = document.getElementById("closeMenu");
  if (!ham || !menu) return;
  const setMenuState = open => { menu.classList.toggle("translate-x-full", !open); menu.setAttribute("aria-hidden", String(!open)); ham.setAttribute("aria-expanded", String(open)); ham.setAttribute("aria-label", open ? "Close navigation" : "Open navigation"); if (open) { const first = menu.querySelector("a, button"); if (first) setTimeout(() => first.focus(), 0); } else ham.focus(); };
  menu.setAttribute("aria-hidden", "true"); ham.setAttribute("aria-expanded", "false");
  ...
  const style = document.createElement("style"); style.id = "lil-synn-navigation-runtime"; style.textContent = `
    ...
    .nav-library-pair { width:100%; display:flex; flex-direction:column; gap:0; }
  ...
  const makeGroup = (id, label, links) => { const group = document.createElement("div"); group.className = "nav-library-group"; const button = document.createElement("button"); button.type = "button"; button.className = "menu-link flex justify-between w-full shrink-0"; button.setAttribute("aria-expanded", "false"); button.setAttribute("aria-controls", id); button.textContent = label; const list = document.createElement("div"); list.id = id; list.className = "hidden flex flex-col gap-3 mt-4 pl-4 text-base font-['Rajdhani'] nav-scroll-library"; links.forEach(([name, href]) => { const a = document.createElement("a"); a.href = href; a.target = "_blank"; a.rel = "noopener noreferrer"; a.className = "menu-link"; a.textContent = name; list.appendChild(a); }); button.addEventListener("click", event => { event.preventDefault(); event.stopPropagation(); const open = list.classList.contains("hidden"); list.classList.toggle("hidden", !open); button.setAttribute("aria-expanded", String(open)); }); group.append(button, list); return group; };
  if (!document.getElementById("streamTrigger")) {
    ...
    if (originalSocialGroup && videosLink) {
      ...
      const socialsGroup = makeGroup("socialsDropdown", "Socials", socialLinks), streamGroup = makeGroup("streamDropdown", "Stream", streamLinks), libraryPair = document.createElement("div");
