document.addEventListener("DOMContentLoaded", () => {
  ...
  if (bgVideo) {
    ...
    if (reducedMotion || saveData || (smallViewport && slowConnection)) { bgVideo.pause(); bgVideo.removeAttribute("autoplay"); }
    else { const startVideo = () => { const p = bgVideo.play(); if (p && typeof p.catch === "function") p.catch(() => {}); }; if (bgVideo.readyState >= 2) startVideo(); else bgVideo.addEventListener("loadeddata", startVideo, { once: true }); }
  }
  ...
  async function loadVideos() {
    ...
    function renderItems(items) {
      const validItems = (items || []).filter(item => item && (item.videoId || (item.id && (item.id.videoId || item.id)))).slice(0, 11);
      grid.innerHTML = validItems.map(item => {
        ...
        const safeTitle = escapeHtml(title); const safeVid = escapeHtml(vid); const youtubeUrl = `https://www.youtube.com/watch?v=${encodeURIComponent(vid)}`;
      ...
      grid.querySelectorAll(".yt-thumb").forEach(img => {
        const vid = img.getAttribute("data-ytid"); const host = img.closest(".yt-frame-host"); if (!vid || !host) return;
        const playVideo = event => {
          if (event) { event.preventDefault(); event.stopPropagation(); }
          const youtubeUrl = `https://www.youtube.com/watch?v=${encodeURIComponent(vid)}`;
          const iframe = document.createElement("iframe"); iframe.className = "youtube-iframe"; iframe.title = `LIL SYNN — ${img.alt}`; iframe.allow = "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"; iframe.allowFullscreen = true; iframe.referrerPolicy = "strict-origin-when-cross-origin"; iframe.loading = "eager"; iframe.style.cssText = "position:absolute;inset:0;width:100%;height:100%;border:0;display:block;background:#000;"; iframe.src = `https://www.youtube-nocookie.com/embed/${encodeURIComponent(vid)}?autoplay=1&playsinline=1&rel=0&modestbranding=1&enablejsapi=1&origin=${encodeURIComponent(window.location.origin)}`;
          host.innerHTML = ""; host.appendChild(iframe);
          const fallback = document.createElement("div"); fallback.style.cssText = "position:absolute;left:0;right:0;bottom:0;z-index:3;text-align:center;padding:8px;background:linear-gradient(transparent,rgba(0,0,0,.9));pointer-events:none;"; fallback.innerHTML = `<a href="${youtubeUrl}" target="_blank" rel="noopener noreferrer" style="pointer-events:auto;color:#fff;text-decoration:underline;font:600 14px Rajdhani,sans-serif;">OPEN ON YOUTUBE</a>`; host.appendChild(fallback); iframe.focus();
        };
        img.addEventListener("click", playVideo); host.querySelector(".yt-play")?.addEventListener("click", playVideo);
      });
    }
    try { const res = await fetch("/api/youtube", { cache: "no-store" }); if (res.ok) { const data = await res.json(); if (Array.isArray(data?.items) && data.items.length) { renderItems(data.items); return; } } } catch (e) { console.warn("YouTube API fetch failed; using fallback catalog."); }
    try { const res = await fetch("/assets/youtube-fallback.json", { cache: "no-store" }); if (res.ok) { const data = await res.json(); if (Array.isArray(data?.items) && data.items.length) { renderItems(data.items); return; } } } catch (e) { console.warn("YouTube fallback fetch failed."); }
