...
assert.ok(rewrites.some(r => r.source === '/suno' && r.destination === '/Suno/Suno_Guide.html'), 'Lowercase /suno must resolve to the Suno guide');
assert.ok(rewrites.some(r => r.source === '/suno/' && r.destination === '/Suno/Suno_Guide.html'), 'Lowercase /suno/ must resolve to the Suno guide');
assert.ok(rewrites.some(r => r.source === '/suno/:path*' && r.destination === '/Suno/:path*'), 'Lowercase Suno child routes must map to the real Suno tree');

console.log('Command Center + Suno route regression harness: PASS');
